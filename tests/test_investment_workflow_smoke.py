...
def test_investment_workflow_cli_smoke_generates_contract_artifacts(tmp_path, monkeypatch, capsys) -> None:
    ...
    assert "open_market_analysis_summary" in dashboard_payload
    assert any(block.get("id") == "open_market_analysis" for block in dashboard_payload["dashboard_v2_blocks"])
    ...
    assert dashboard_payload["cards"][0]["evidence_action_summary"]["rationale"]
    assert "global_evidence_action_summary" in dashboard_payload["cards"][0]
    assert "US" in dashboard_payload["market_views"]
    assert "US" in dashboard_payload["market_evidence_action_summary"]
    assert "evidence_focus_actions" in dashboard_payload
    assert "evidence_focus_summary" in dashboard_payload
    assert dashboard_payload["evidence_focus_summary"]["read_only"] is True
    assert dashboard_payload["market_views"]["US"]["evidence_action_label"]
