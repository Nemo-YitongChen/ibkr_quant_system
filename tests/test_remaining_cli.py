...
def test_remaining_cli_parse_args_accepts_explicit_argv() -> None:
    ...
    assert run_investment_execution.parse_args(["--market", "HK", "--submit"]).submit is True
    assert run_investment_guard.parse_args(["--market", "US", "--submit"]).submit is True
    assert run_investment_opportunity.parse_args(["--market", "US", "--request_timeout_sec", "5"]).request_timeout_sec == 5.0
...
def test_remaining_cli_summary_payloads() -> None:
    ...
    execution_summary, execution_artifacts = run_investment_execution._cli_summary_payload(
        SimpleNamespace(market="HK", portfolio_id="HK:watchlist", submitted=True, order_count=5, gap_symbols=2, gap_notional=123.45),
        Path("reports/hk"),
    )
    ...
    guard_summary, guard_artifacts = run_investment_guard._cli_summary_payload(
        SimpleNamespace(market="US", portfolio_id="US:market_us", submitted=False, order_count=4, stop_count=3, take_profit_count=1),
        Path("reports/us"),
    )
    ...
    opp_summary, opp_artifacts = run_investment_opportunity._cli_summary_payload(
        SimpleNamespace(market="US", portfolio_id="US:market_us", entry_now_count=2, near_entry_count=5, wait_count=8),
        Path("reports/us"),
    )
