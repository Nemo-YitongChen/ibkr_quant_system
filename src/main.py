...
try:
    ...
    from .common.markets import add_market_args, load_market_universe_config, load_symbols_from_symbol_master, market_config_path, resolve_market_code
    ...
    from .risk.limits import RiskContextConfig
    ...
    from .portfolio.allocator import PortfolioAllocator, AllocatorConfig
...
def _load_runtime_borrow_fee_maps(
    ...
    if short_borrow_fee_file:
        try:
            fee_path = resolve_path(short_borrow_fee_file)
            ...
            storage.insert_risk_event(
                ...
                0.0,
                f"path={short_borrow_fee_file} error={type(e).__name__}",
            )
            log.warning(f"borrow fee file load failed: path={short_borrow_fee_file} error={type(e).__name__} {e}")
...
def _load_runtime_short_safety_rules(
    ...
    if short_safety_file:
        try:
            rules_path = resolve_path(short_safety_file)
            ...
            storage.insert_risk_event(
                ...
                0.0,
                f"path={short_safety_file} error={type(e).__name__}",
            )
            log.warning(f"short safety file load failed: path={short_safety_file} error={type(e).__name__} {e}")
    ...
    *,
    ...
    return StrategyConfig(
        ...
        paper_allowed_execution_sources=[
            str(x).upper()
            for x in list(paper_allowed_execution_sources or ["REALTIME"])
            if str(x).strip()
        ],
...
def _run_startup_self_check(
    ...
    *,
    ...
    uptick_count = sum(1 for val in dict(short_safety_raw.get("has_uptick_data", {}) or {}).values() if bool(val))
    ...
    if warnings:
        log.warning(f"startup self-check warnings: {', '.join(warnings)}")
    else:
...
def main():
    ...
    ibkr_cfg = load_yaml(ibkr_cfg_path)
    market_universe_cfg = load_market_universe_config(BASE_DIR, market_code)
    risk_cfg = load_yaml(str(ibkr_cfg.get("risk_config", "config/risk.yaml")))
    strat_cfg_raw = load_yaml(str(ibkr_cfg.get("strategy_config", "config/strategy_defaults.yaml")))
    regime_adaptor_cfg_raw = load_yaml(str(ibkr_cfg.get("regime_adaptor_config", "config/regime_adaptor.yaml")))
    ...
    symbol_master_path = resolve_path(symbol_master_db)
    symbol_master_symbols = load_symbols_from_symbol_master(symbol_master_path, market_code)
    if symbol_master_symbols:
        log.info(f"Loaded live symbol master: market={market_code or 'DEFAULT'} path={symbol_master_path} symbols={len(symbol_master_symbols)}")
    ...
    if market_code:
        log.info(f"Using market universe config: {BASE_DIR / 'config' / 'markets' / market_code.lower() / 'universe.yaml'}")
    if watchlist_yaml and not symbol_master_symbols:
        watchlist_path = resolve_path(str(watchlist_yaml))
        watchlist_symbols = load_watchlist_symbols(str(watchlist_path))
        if watchlist_symbols:
            log.info(f"Loaded seed watchlist: market={market_code or 'DEFAULT'} config={ibkr_cfg_path} path={watchlist_path} symbols={len(watchlist_symbols)}")
