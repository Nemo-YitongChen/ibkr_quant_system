...
class InvestmentOpportunityEngine:
    def __init__(
        ...
        *,
        ...
        self.opportunity_cfg = opportunity_cfg
    ...
    def _write_md(path: Path, summary: Dict[str, Any], rows: List[Dict[str, Any]]) -> None:
        ...
        if not rows:
            ...
            for row in rows:
                lines.append(
                    f"- **{row['symbol']}** status={row['entry_status']} action={row['action']} "
                ...
                lines.append(f"  {row.get('entry_reason', '')}")
    ...
    def run(self, *, report_dir: str) -> InvestmentOpportunityResult:
        ...
        priority = {"ENTRY_NOW": 0, "ADD_ON_PULLBACK": 1, "NEAR_ENTRY": 2, "WAIT_PULLBACK": 3, "WAIT_TREND": 4}
