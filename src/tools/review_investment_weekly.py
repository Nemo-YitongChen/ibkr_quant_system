...
def _load_feedback_threshold_overrides(path: Path) -> Dict[str, Dict[str, Dict[str, float]]]:
    raw = _read_yaml(path)
...
def _feedback_automation_thresholds(
    feedback_kind: str,
    *,
    market: str = "",
    ...
    if overrides:
        # 这里按市场覆写 AUTO_APPLY 阈值，让 paper 可以先试运行更细粒度的门槛，
        # live 仍保留人工确认，不会因为建议存在就自动放开。
        base.update(overrides)
    return base


def _feedback_action_field(feedback_kind: str) -> str:
    kind = str(feedback_kind or "").strip().lower()
    if kind == "shadow":
...
def _feedback_kind_label(feedback_kind: str) -> str:
    kind = str(feedback_kind or "").strip().lower()
    if kind == "shadow":
        return "Shadow ML"
    if kind == "risk":
...
def _feedback_automation_mode_label(mode: str) -> str:
...
def _feedback_automation_basis_label(basis: str) -> str:
...
def _feedback_maturity_label(ratio: float, pending_count: int, sample_count: int) -> str:
...
def _feedback_threshold_effect_bucket(text: Any) -> str:
...
def _build_feedback_threshold_tuning_summary(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    out: List[Dict[str, Any]] = []
    for raw in list(rows or []):
        row = dict(raw)
        latest_bucket = _feedback_threshold_effect_bucket(row.get("latest_effect"))
        w1_bucket = _feedback_threshold_effect_bucket(row.get("effect_w1"))
        w2_bucket = _feedback_threshold_effect_bucket(row.get("effect_w2"))
        w4_bucket = _feedback_threshold_effect_bucket(row.get("effect_w4"))
        ...
        # 这里给的是“更明确的调参方向”，仍然先作为周报/看板建议，不直接改 live。
...
def _build_feedback_threshold_suggestion_rows(
    rows: List[Dict[str, Any]],
    threshold_overrides: Dict[str, Dict[str, Dict[str, float]]] | None = None,
) -> List[Dict[str, Any]]:
    out: List[Dict[str, Any]] = []
    for raw in list(rows or []):
        ...
        if not feedback_kind:
            continue
        base = _feedback_automation_thresholds(
            feedback_kind,
            market=str(row.get("market") or ""),
