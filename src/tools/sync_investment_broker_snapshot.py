...
log = get_logger("tools.sync_investment_broker_snapshot")
BASE_DIR = Path(__file__).resolve().parents[2]


def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Sync current IBKR broker holdings into the scoped investment runtime.")
    add_market_args(ap)
    ...
    ap.add_argument("--report_dir", default="", help="Explicit report directory used for this portfolio.")
...
def main() -> None:
    ...
    try:
        ...
        summary = engine.sync_broker_snapshot(report_dir=str(report_dir))
        print(
            f"market={summary['market']} portfolio={summary['portfolio_id']} "
            f"account={summary['account_id']} positions={summary['position_count']}"
        )
        print(f"summary_json={report_dir / 'investment_broker_snapshot_summary.json'}")
    finally:
        try:
            ib.disconnect()
        except Exception:
