...
当前分支已经不再只是 PR A 文档交接，而是把 PR A 后续切片推进到代码层：

- `src/common/dashboard_evidence.py` 与 `tests/test_dashboard_evidence.py` 已存在，并继续补了 US/HK/CN market context。
- dashboard v2 blocks 已接入 advanced HTML，并补充 metrics 展示。
- dashboard control action audit 已推进到独立 JSONL 持久化与脱敏。
- weekly unified evidence / blocked-vs-allowed review 已输出 5/20/60d 多周期判断字段。

因此，PR #10 如果继续保留，应更新为“dashboard evidence implementation follow-up”，不要再按旧文档分支覆盖当前 main 的 helper refactor。
